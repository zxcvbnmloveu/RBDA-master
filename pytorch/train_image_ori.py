...
import lr_schedule, pre_process as prep, network, loss_ori
...
def save_feature(loader, model):
    ...
    with torch.no_grad():
        ...
        np.save(log_name+'DANN_t.npy',all_feature)
        np.save(log_name+'DANN_t_l.npy',all_label)
    start_test = True
    ...
    with torch.no_grad():
        ...
        all_feature = all_feature.cpu().numpy()
        all_label = all_label.cpu().numpy()
        np.save(log_name+'DANN_s.npy', all_feature)
        np.save(log_name+'DANN_s_l.npy', all_label)
...
def train(config):
    ...
    # base_network = nn.DataParallel(base_network,device_ids=[0,1])
    base_network = base_network.cuda()
    # if isinstance(base_network, torch.nn.DataParallel):
    ...
    if config["loss"]["random"]:
        ...
        ad_net = network.AdversarialNetwork(base_network.output_num(), 1024)#DANN
    ...
    for i in range(config["num_iterations"]+1):
        if i % config["test_interval"] == config["test_interval"] - 1:
            ...
            # output(s_centroid.__str__())
            # output(t_centroid.__str__())
            output(log_str)
        if i % 1000 == 0:
            print('save')
            save_feature(dset_loaders,base_network)
        ...
        if config['method'] == 'CDAN+E':
            entropy = loss_ori.Entropy(softmax_out)
            transfer_loss = loss_ori.CDAN([features, softmax_out], ad_net, entropy, network.calc_coeff(i), random_layer)
        elif config['method'] == 'CDAN':
            transfer_loss = loss_ori.CDAN([features, softmax_out], ad_net, None, None, random_layer)
        elif config['method'] == 'DANN':
            transfer_loss = loss_ori.DANN([features_source, features_target, outputs_source, outputs_target],
                                                    ad_net,labels_source=labels_source,n_class = config["network"]["params"]["class_num"])
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--method', type=str, default='CDAN+E', choices=['CDAN', 'CDAN+E', 'DANN','Base'])
    ...
    config["data"] = {"source": {"list_path": args.s_dset_path, "batch_size": 18},#原本36
                      "target": {"list_path": args.t_dset_path, "batch_size": 18},
    ...
    log = open('./snapshot/' + log_name + '.log', 'w')
