...
import sys
import time
from util_MT import ConditionalEntropyLoss, VAT, compute_aug_loss2, CosineMarginLoss, compute_cbloss
import numpy as np
import torch
...
def image_classification_test(loader, model, test_10crop=False,n_class = 12):
    ...
    print([pre[i]/real[i] for i in range(n_class)])
...
def train(config):
    ...
    for i in range(max_iter+1):
        if i % config["test_interval"] == config["test_interval"] - 1 and i > 6000:
        ...
        if config['method'] == 'DANN+dis' or config['method'] == 'CDRL':
        ...
        elif config['method'] == 'CDAN+MT+cent':
            ...
            loss_trg_cent = 1e-2 * cent(outputs_target_stu)
            loss_trg_vat = 1e-2 * vat_loss(inputs_target, outputs_target_stu)
            classifier_loss = nn.CrossEntropyLoss(weight=weight)(outputs_source/1.05, labels_source)
            # classifier_loss = nn.CrossEntropyLoss()(outputs_source, labels_source)
            transfer_loss = loss_params["trade_off"] * ad_loss \
                + 0.001*(unsup_loss + loss_trg_vat + loss_trg_cent)
